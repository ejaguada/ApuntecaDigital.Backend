...
To update the database use this command from the Web project folder (replace `ApuntecaDigital.Backend` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../ApuntecaDigital.Backend.Infrastructure/ApuntecaDigital.Backend.Infrastructure.csproj -s ApuntecaDigital.Backend.Web.csproj
```
